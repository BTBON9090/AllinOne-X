      ...
      for (const node of pool) {
        ...
        // -------------------------------------------------
                        ...
                        if (match && f.types.vals.length > 0) {
                            ...
                            // 1. 基础类型直接匹配 (RECTANGLE, TEXT, GROUP, FRAME, COMPONENT, INSTANCE...)
                            ...
                            if (ts.includes('IMAGE') && 'fills' in node && Array.isArray(node.fills)) {
                                if (node.fills.some(paint => paint.type === 'IMAGE')) isType = true;
                            }
                            
                            // 变体集 (Component Set)
                            if (ts.includes('COMPONENT_SET') && t === 'COMPONENT_SET') isType = true;
                            
                            // 形状文字 (Shape with Text - FigJam)
                            if (ts.includes('SHAPE_WITH_TEXT') && t === 'SHAPE_WITH_TEXT') isType = true;
                            
                            // 贴纸 (Sticky)
                            if (ts.includes('STICKY') && t === 'STICKY') isType = true;
                            
                            // 连接线 (Connector)
                            if (ts.includes('CONNECTOR') && t === 'CONNECTOR') isType = true;
                        ...
                        if (match && f.states.vals.length > 0) {
                            ...
                            // 无填充 (No Fill)
                            // 注意：有些节点没有 fills 属性 (如 Group)，需要排除
                            ...
                            // 空容器 (No Children / Empty)
                            // 适用于 Frame, Group, Component, Instance, Section 等容器
    ...
    case 'locate-node': {
        try {
            ...
            if (node && node.type === 'TEXT') {
                ...
                // 2. 精确高亮逻辑 (修复Bug 2: 仅标红关键字，不含项目符号)
                if (typeof msg.index === 'number' && typeof msg.length === 'number') {
                    ...
                    try {
                        ...
                        // 🌟 关键：只改变特定 range 的颜色，绝对不会影响 bullet point
                        // 因为 index 是正则算出来的，绝对准确
                        node.setRangeFills(msg.index, msg.index + msg.length, highlightPaint);
                        
                        figma.notify("已定位并标红选中项 (Ctrl+Z 可撤销)");
                    } catch (fontErr) {
                        figma.notify("已定位 (字体加载失败，无法标红)");
                    }
                }
            } else {
                figma.notify("图层不存在或不是文本");
            }
        } catch (e) {
            console.error(e);
        }

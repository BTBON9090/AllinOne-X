...
figma.ui.onmessage = async (msg) => {
  ...
  switch (msg.type) {
    ...
    // =============================================================
    // 查找并选择 (Find & Select) - 完整修复版
    // =============================================================
    case 'find-and-select': {
        const f = msg.filters;
        console.log("正在执行查找，筛选条件:", f); // 调试日志
        ...
        if (f.scope === 'inside') {
            // "内在元素": 必须先选中容器
            if (sel.length === 0) {
                figma.notify("请先选择一个容器 (画板/组) 以查找其内部元素");
                return;
            }
            // 遍历选中节点的内部
            for (const node of sel) {
                if ('findAll' in node) {
                    pool = pool.concat(node.findAll(() => true));
                }
            }
        } 
            ...
            if (sel.length > 0 && sel[0].parent) {
                ...
                figma.notify("请先选择一个图层以查找其同级");
                return;
            }
        } 
        else {
            // "子孙元素" (Descendants) 或 默认情况
            // 如果没选图层，默认查整个页面；如果选了，查选中图层及其子孙
            const targets = sel.length > 0 ? sel : [figma.currentPage];
            
            for (const node of targets) {
                // 如果是 Page，不用加自己；如果是图层，先把图层本身加进去(可选，这里只查内部)
        ...
        for (const node of pool) {
            ...
            if (f.name && f.name.val) {
                ...
                if (f.name.caseSensitive) {
                    ...
                    if (!n.toLowerCase().includes(q.toLowerCase())) match = false;
                }
            }

            // --- B. 类型匹配 (Type) ---
            if (match && f.types && f.types.vals && f.types.vals.length > 0) {
                const t = node.type;
                const ts = f.types.vals; // 前端传来的类型数组
                let isType = false;

                // 1. 直接匹配 (RECTANGLE, TEXT, GROUP, etc.)
            ...
            // --- C. 状态匹配 (State) ---
            if (match && f.states && f.states.vals && f.states.vals.length > 0) {
                let isState = false;
            ...
            if (match && f.props && f.props.length > 0) {
                for (const p of f.props) {
                    ...
                    else if (p.key in node) val = node[p.key];
                    
                    // 特殊属性映射
                    if (p.key === 'fillCount' && 'fills' in node) val = node.fills.length;
                    if (p.key === 'strokeCount' && 'strokes' in node) val = node.strokes.length;

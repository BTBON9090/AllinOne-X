...
AllinoneX 4.1.1 是同仓库双平台插件项目。Figma 与 MasterGo 版本分别保留独立的主线程源码、清单、构建产物和依赖配置，公共功能通过自动等价检查保持同步。
...
- 查重说明补充版本对比、页面迁移、图标检查和组件库清理场景。
- AI 组件规范优化增加响应兼容、超时控制、重试和并发保护。
- 智能填充自定义字段改为标题与操作分两行，适配窄面板。

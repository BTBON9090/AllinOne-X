...
// 收集文本节点
function collectTextNodes(root: BaseNode): TextNode[] {
...
// 安全设置文本
async function setTextSafe(node: TextNode, text: string): Promise<boolean> {
  try {
...
// 验证选择
function validateSelection(minCount: number = 1): boolean {
...
// 记录操作
function logOperation(operation: string, details?: Record<string, any>): void {
  ...
  try {
    ...
    switch (msg.type) {
      ...
      case 'get-selection-count': {
        const textNodes = collectTextNodes(
          figma.currentPage.selection.length > 0
            ? { children: figma.currentPage.selection } as any
            : figma.currentPage
        )
      ...
      case 'smart-fill-exec': {
        ...
        const sortedNodes = sortNodesByPosition(textNodes) as TextNode[]

        let successCount = 0

        await batchExecute(
          sortedNodes,
          async (node: TextNode, index: number) => {
            let textToFill = ''
      ...
      case 'to-frame': {
        ...
        for (const node of selection) {
          ...
          if ('fills' in node && Array.isArray(node.fills)) frame.fills = node.fills
      ...
      case 'to-rect': {
        ...
        for (const node of selection) {
          if ((node.type === 'FRAME' || node.type === 'GROUP') && !node.removed) {
            ...
            if ('fills' in node && Array.isArray(node.fills)) rect.fills = node.fills
            if ('strokes' in node && Array.isArray(node.strokes)) {
      ...
      case 'select-text': {
        ...
        for (const node of pool) {
          ...
          if ('findAll' in node) {
            const found = node.findAll((n) => n.type === 'TEXT') as TextNode[]
      ...
      case 'remove-al': {
        ...
        function removeAutoLayout(node: any) {
          if (node.layoutMode && node.layoutMode !== 'NONE') {
            node.layoutMode = 'NONE'
            count++
          }
          if (node.children) node.children.forEach(removeAutoLayout)
        }
      ...
      case 'rename-content': {
        ...
        for (const node of selection) {
          ...
          if (name) node.name = name.substring(0, 20)
        }
      ...
      case 'remove-hidden': {
        const selection = figma.currentPage.selection
        const pool = selection.length > 0 ? selection : [figma.currentPage]
        let hiddenNodes: SceneNode[] = []

        for (const node of pool) {
          if ('visible' in node && !node.visible) hiddenNodes.push(node as SceneNode)
          if ('findAll' in node) {
            const found = node.findAll((n) => !n.visible) as SceneNode[]
      ...
      case 'ungroup-all': {
        ...
        for (const node of selection) {
          if ('findAll' in node) {
            let groups = node.findAll((n) => n.type === 'GROUP') as GroupNode[]
            while (groups.length > 0) {
              ...
              groups = node.findAll((n) => n.type === 'GROUP') as GroupNode[]
            }
          }
        }
      ...
      case 'unlock-all': {
        ...
        function unlock(node: any) {
          if ('locked' in node && node.locked) {
            node.locked = false
            count++
          }
          if ('children' in node) node.children.forEach(unlock)
        }
      ...
      case 'find-and-select': {
        ...
        if (filters.scope === 'page') {
          ...
          selection.forEach(node => {
            if ('findAll' in node) {
              allNodes.push(...(node.findAll(() => true) as SceneNode[]))
            }
          })
          pool = allNodes
          ...
          selection.forEach(node => {
            if ('children' in node) {
              node.children.forEach(child => {
                ...
                if ('findAll' in child) {
                  allNodes.push(...(child.findAll(() => true) as SceneNode[]))
                }
              })
            }
          })
          ...
          selection.forEach(node => {
            if ('children' in node) {
              allNodes.push(...(node.children as SceneNode[]))
            }
          })
          pool = allNodes
          ...
          selection.forEach(node => {
            if (node.parent && 'children' in node.parent) {
              allNodes.push(...(node.parent.children.filter(n => n !== node) as SceneNode[]))
            }
          })
          pool = allNodes
        } else {
        ...
        if (filters.states && filters.states.vals.length > 0) {
          ...
          results = results.filter(node => {
            ...
            if (stateSet.has('hidden')) matchState = matchState || !node.visible
            if (stateSet.has('locked')) matchState = matchState || node.locked
            if (stateSet.has('mask') && 'isMask' in node) matchState = matchState || node.isMask
            if (stateSet.has('export')) matchState = matchState || node.exportSettings.length > 0
            if (stateSet.has('clip') && 'clipsContent' in node) matchState = matchState || node.clipsContent
